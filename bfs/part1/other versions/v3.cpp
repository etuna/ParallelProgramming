...
int mpi_vertex_dist(graph_t *graph, int start_vertex, int *result)
{
    int num_vertices = graph->num_vertices;
    fill_n(result, num_vertices, MAX_DIST);
    int *s_result = new int[num_vertices + 1];
    int i;
    int depth = 0;
    result[start_vertex] = depth;
    int keep_going = true;
    ...
    s_result[num_vertices] = keep_going;

    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    auto start_time = Time::now();
    while (s_result[num_vertices] == -1)
    {
        s_result[num_vertices] = 0;
        for (int vertex = 0; vertex < num_vertices; vertex += size)
        {
            int mVertex = vertex + rank;

            if (s_result[mVertex] == depth)
            {
                for (int n = graph->v_adj_begin[mVertex];
                     n < graph->v_adj_begin[mVertex] + graph->v_adj_length[mVertex]; n++)
                {
                    ...
                    if (s_result[neighbor] > depth + 1)
                    {
                        s_result[neighbor] = depth + 1;
                        s_result[num_vertices]=-1;
                    }
                }
            }
        }
        MPI_Allreduce(MPI_IN_PLACE, s_result, num_vertices + 1, MPI_INT, MPI_MIN, MPI_COMM_WORLD);
        depth++;
    }
    for (i = 0; i < num_vertices; i++)
    {
