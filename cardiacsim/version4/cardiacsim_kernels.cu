  ...
  const double M1,const double  M2, const double b)
{
  ...
  __shared__ double E_Block[32][32];
  __shared__ double R_Block[32][32];
...
if ((row - 1 < m) && (col - 1 < n)) {
  ...
  E_Block[threadIdy][threadId] = E[index];
  R_Block[threadIdy][threadId] = R[index];
  
  E_Block[threadIdy][threadId] = E_Block[threadIdy][threadId] - dt*(kk*E_Block[threadIdy][threadId]*(E_Block[threadIdy][threadId] - a)*(E_Block[threadIdy][threadId] - 1) + E_Block[threadIdy][threadId]*R_Block[threadIdy][threadId]);
  E[index] = E_Block[threadIdy][threadId];

  R[index] = R_Block[threadIdy][threadId] + dt*(epsilon + M1*R_Block[threadIdy][threadId]/(E_Block[threadIdy][threadId] + M2))*(-R_Block[threadIdy][threadId] - kk*E_Block[threadIdy][threadId]*(E_Block[threadIdy][threadId] - b - 1));
  }
}
 using namespace std;
