...
__global__ void _handlePaddingRegions(double *E_prev, const int n, const int m)
{
  ...
  /** Checking the variables **/
   //if(E_prev[index] != 0)
  //printf("eprev index:%d", E_prev[index]);
  // printf("_handlePaddingRegionssecondly index:%d\n",  E_prev[index]);
  // printf("_handlePaddingRegionssecondly m index:%d\n",  m);
  // printf("_handlePaddingRegionssecondly  n index:%d\n", n);
  
  if (threadId == 0) {
    E_prev[index] = E_prev[index + 2];
    E_prev[index + n + 1] = E_prev[index + n - 1];
			       ...
			       const int n, const int m)
{
  ...
  /** Checking the variables **/
  // printf("_solvePDEsecondly index:%d\n",  E[index]);
  // printf("_solvePDEsecondly index:%d\n",  E_prev[index]);
  // printf("_solvePDEsecondly alpha index:%d\n",  alpha);
  // printf("_solvePDEsecondly  n index:%d\n", n);
  // printf("_solvePDEsecondly m index:%d\n",  m);
 ...
 int main (int argc, char** argv)
 {
  ...
  //   }
   ...
   while (t<T) {
     ...
     /* Kernel Launch 1 : Handling padding regions */
     _handlePaddingRegions<<<grids, threads>>>(d_E_prev, n, m);
      ...
      /* Kernel Launch 2: Solving PDEs */
     _solvePDE<<<grids, threads>>>(d_E,  d_E_prev, alpha, n, m);
      ...
      /* Kernel Launch 3: Solving ODEs */
     _solveODE<<<grids, threads>>>(d_E, d_R, alpha, n, m, kk, dt, a, epsilon, M1, M2, b);
      /* Used For printf */
      //cudaDeviceSynchronize();
